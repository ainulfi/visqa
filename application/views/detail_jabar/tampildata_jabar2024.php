...
<div class="col-lg-12">
    <div class="card m-b-30">
        <h4 class="card-header mt-0">Detail Data Rapor Pendidikan Provinsi Jawa Barat Tahun 2024</h4>
        <?= form_open('rp2024_jabar/deletemultiple', ['class' => 'formhapus']) ?>
        <div class="card-body">
            ...
            <p class="card-text">
                <table class="table table-bordered table-striped display nowrap" style="width:100%;" id="data_rpjabar2024">
                    <thead>
                        <tr>
                            <th>
...
function tampildatarapor() {
    table = $('#data_rpjabar2024').DataTable({
        responsive: true,
        "destroy": true,
        "processing": true,
        "serverSide": true,
        ...
        "ajax": {
            "url": "<?= site_url('rp2024_jabar/ambildata') ?>",
            "type": "POST"
        },
...
function lihat(npsn) {
    $.ajax({
        type: 'post',
        url: "<?= site_url('rp2024_jabar/lihatdata') ?>",
        data: {
            npsn: npsn
        },
        dataType: "json",
...
function edit(npsn) {
    $.ajax({
        type: 'post',
        url: "<?= site_url('rp2024_jabar/formedit') ?>",
        data: {
            npsn: npsn
        },
        dataType: "json",
...
function hapus(id) {
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                type: "post",
                url: "<?= site_url('rp2024_jabar/hapus') ?>",
                data: {
                    id: id,
                },
                dataType: "json",
