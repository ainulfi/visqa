...
<div class="col-lg-12">
    <div class="card m-b-30">
        <h4 class="card-header mt-0">Detail Data Rapor Pendidikan Provinsi Lampung Tahun 2024</h4>
        <?= form_open('rp2024_lampung/deletemultiple', ['class' => 'formhapus']) ?>
        <div class="card-body">
            ...
            <p class="card-text">
                <table class="table table-bordered table-striped display nowrap" style="width:100%;" id="data_rplampung2024">
                    <thead>
                        <tr>
                            <th>
...
function tampildatarapor() {
    table = $('#data_rplampung2024').DataTable({
        responsive: true,
        "destroy": true,
        "processing": true,
        "serverSide": true,
        ...
        "ajax": {
            "url": "<?= site_url('rp2024_lampung/ambildata') ?>",
            "type": "POST"
        },
...
$(document).ready(function() {
    ...
    $('#tomboltambah').click(function(e) {
        $.ajax({
            url: "<?= site_url('rp2023_lampung/formtambah') ?>",
            dataType: "json",
...
function lihat(npsn) {
    $.ajax({
        type: 'post',
        url: "<?= site_url('rp2024_lampung/lihatdata') ?>",
        data: {
            npsn: npsn
        },
        dataType: "json",
...
function edit(npsn) {
    $.ajax({
        type: 'post',
        url: "<?= site_url('rp2024_lampung/formedit') ?>",
        data: {
            npsn: npsn
        },
        dataType: "json",
...
function hapus(id) {
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                type: "post",
                url: "<?= site_url('rp2024_lampung/hapus') ?>",
                data: {
                    id: id,
                },
                dataType: "json",
